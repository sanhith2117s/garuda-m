  ...
  return (
    <group ref={group}>
      <mesh rotation={[0.3, 0.4, 0.1]}>
        ...
        <MeshTransmissionMaterial backside thickness={0.62} roughness={0.12} transmission={0.98} ior={1.35} chromaticAberration={0.08} color="#dca86b" />
      </mesh>
      ...
      <mesh scale={1.78} rotation={[1.2, 0.15, 0.4]}>
        ...
        <meshBasicMaterial color="#dca86b" transparent opacity={0.42} />
      </mesh>
    </group>
  );
}
...
function HeroScene() {
  return (
    <Canvas camera={{ position: [0, 0, 5.8], fov: 42 }} dpr={[1, 1.7]} gl={{ antialias: true, alpha: true }}>
      ...
      <pointLight position={[3, 4, 4]} intensity={14} color="#dca86b" />
      <pointLight position={[-4, -2, 2]} intensity={10} color="#6aa99b" />
      <Suspense fallback={null}>
        <Float speed={1.4} rotationIntensity={0.2} floatIntensity={0.48}>
          <OrbitalCore />
        </Float>
        <DreiSparkles count={85} scale={7} size={1.8} speed={0.28} color="#dca86b" />
...
export default function LandingPage() {
  ...
  const links = useMemo(() => [['About', 'about'], ['System', 'system'], ['Signals', 'signals'], ['Institutions', 'institutions']], []);
  ...
  return (
    <main className="garuda-home bg-[#f5f2eb] text-[#101820] dark:bg-[#0b1117] dark:text-[#f5f0e7] overflow-hidden">
      <header className="fixed top-0 inset-x-0 z-50 px-4 sm:px-8 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between rounded-full border border-slate-900/10 dark:border-white/10 bg-[#f5f2eb]/75 dark:bg-[#0b1117]/75 backdrop-blur-xl px-4 sm:px-6 py-3 shadow-[0_12px_60px_rgba(0,0,0,.08)]">
          <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="flex items-center gap-3" aria-label="Back to top">
            <span className="grid size-9 place-items-center rounded-full bg-[#dca86b] text-[#101820]"><ShieldCheck size={18} /></span>
          ...
          <nav className="hidden md:flex items-center gap-7 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
            {links.map(([label, id]) => <button key={id} onClick={() => document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' })} className="hover:text-[#dca86b] transition-colors">{label}</button>)}
          </nav>
      ...
      <section id="hero" className="relative min-h-[100svh] flex items-center pt-28 pb-16">
        ...
        <div className="absolute -top-32 -right-32 size-[36rem] rounded-full bg-[#dca86b]/20 blur-[110px]" />
        <div className="relative z-10 max-w-7xl w-full mx-auto px-6 sm:px-10 grid lg:grid-cols-[1.05fr_.95fr] gap-8 items-center">
          <div className="max-w-3xl">
            <Reveal><p className="mb-6 flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.34em] text-[#6b9f94]"><span className="h-px w-10 bg-[#dca86b]" /> Smart campus infrastructure</p></Reveal>
            <Reveal delay={100}><h1 className="display-title text-[clamp(3.8rem,9vw,8.6rem)] font-black leading-[.84] tracking-[-0.075em]">Move with<br /><span className="text-[#dca86b]">clarity.</span></h1></Reveal>
